...
for i in range(0, len(A)):
    _dir, _sep, _name = A[i].rpartition('/')
    _name, _sep, _ext = _name.rpartition('.')
...
if not os.path.isdir(word_save_dir):
...
f = open(txt_save_dir+'image_files.txt', 'w')
for item in image_files:
  f.write("%s\n" % item)
f.close()

print 'saving word names'
f = open(txt_save_dir+'word_files.txt', 'w')
